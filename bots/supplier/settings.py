...
def lang_settings(update, context):
    ...
    user = supplierservice.get_object_by_user_id(user_id=update.message.chat.id)
    if data == "set_lang_uz":
        user.lang = "uz"
        user.save()
...
def phone_settings(update, context):
    ...
    is_available = Supplier.objects.filter(phone=phone_number)
    if is_available:
        update.message.reply_text(get_word("number is logged", update))
        return PHONE_SETTINGS
    obj = supplierservice.get_object_by_user_id(user_id=update.message.chat.id)
